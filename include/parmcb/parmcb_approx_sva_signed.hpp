#ifndef PARMCB_DETAIL_SPANNER_HPP_
#define PARMCB_DETAIL_SPANNER_HPP_
...
public:
    ...
    WeightType run(CycleOutputIterator out) {
        ...
        EdgeWeightMapType spanner_weight_map = get(boost::edge_weight,
                _spanner);
        _weight += parmcb::mcb_sva_signed(_spanner, spanner_weight_map, out);
...
typename boost::property_traits<WeightMap>::value_type approx_mcb_sva_signed(
    ...
    parmcb::BaseApproxSpannerAlgorithm<Graph, WeightMap> spanner(g, weight, k);
    return spanner.run(out);
}
